...
4.  MySQL 5.1.31 и выше. Вы также можете использовать MySQL совместимые базы данных, например MariaDB;
...
10.  значение `upload_max_filesize` не ниже 2M;
11.  значение `post_max_size` - не ниже 2M и больше значения `upload_max_filesize` (критично для процесса установки);
12.  директива `magic_quotes_runtime` должна быть отключена;
13.  [DocBlock](http://www.phpdoc.org/) должен быть установлен;
14.  параметр `memory_limit` должен быть не ниже 128 МБ (если ваш сервер используете 64-битный процессор, тогда `memory_limit` должен быть 256 МБ и выше);
...
3.  Распакуйте дистрибутив X-Cart в какую-нибудь папку вашего сервера. Вы можете сделать это, используя следующую команду для командной строки в Панели Управления:
    ...
    ```php
    unzip x-cart-5.3.2.11-ru.zip
    ```

    для tgz-архива: 

    ```php
    tar -xzpf x-cart-5.3.2.11-ru.tgz
    ```
