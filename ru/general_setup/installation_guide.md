...
4.  MySQL версии 5.1.31 или новее. Также, подойдет совместимая с MySQL систему MariaDB;
...
15.  {% link "Setting the time limit of your server" ref_xqnpttd4 %}, настроенное в соответствие с конфигурацией сервера;
16.  
17.  такие компоненты как mod_rewrite должны быть включены, чтобы {% link "правильно работали ЧПУ" ref_Vn1mMUw9 %}.
18.  при использовании OPcache директивы `opcache.save_comments` и `opcache.load_comments` должны быть включены. Любые другие кэшеры не должны вырезать комментарии из кода.
